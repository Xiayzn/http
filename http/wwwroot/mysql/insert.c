...
int ParseQueryString(char querystring[], char* factory_name[], int* ItemNo,
	...
	if (tok_size != 8){//如果切分结果不等于8，就认为当前首行有问题
...
int main(){
    ...
    if(ret == 0){
        ...
        printf("<a href=\"javascrip:void(0)\" onclick=\"window.open('http://182.254.244.52:9090/mysql/index.html',\'','height=529, width=700, top=265,left=645, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')\" target=\"_self\" >插入成功</a>");
    }else{

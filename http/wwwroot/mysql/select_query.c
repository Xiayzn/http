...
int ParseQueryString(char querystring[], char* search_name[], char* search_value[]){
    ...
    int tok_size = Split(querystring, "=", tok, 10);                                                                   
    if (tok_size != 2){//如果切分结果不等于4，就认为当前首行有问题                                            
      fprintf(stderr, "Split failed! tok_size=%d\n", tok_size);//打印错误日志                                         
      return -1;//表示切分失败                                                                                         
    }

    //fprintf(stderr, "atoi(%E6%9C%B1) = %d\n", atoi(tok[1]));
    if(atoi(tok[1]) > 0){
      *search_name = "ItemNo";
      *search_value = tok[1];
      return 0;
...
int main(){
    ...
    ParseQueryString(buf, &search_name, &search_value);
    fprintf(stderr, "[CGI] search_name=%s search_value=%s \n", search_name, search_value);
    /////////////////////////////////////////////////
    // 接下来就要进行数据库的查找
    ...
    fprintf(stderr, "[CGI22] search_name=%s search_value=%s  \n", search_name, search_value);                 
    sprintf(sql,               
           "select factory_name, ItemNo, t_date from Manifest where %s='%s'",
            search_name, search_value);//期望用户通过浏览器查询到的数据 
